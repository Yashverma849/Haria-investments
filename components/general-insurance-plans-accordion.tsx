...
import type { InsurancePlanWithCategory } from "@/lib/general-insurance-data";
import { scheduleConsultation } from "@/lib/nav-links";

type GeneralInsurancePlansAccordionProps = {
  plans: InsurancePlanWithCategory[];
};

export default function GeneralInsurancePlansAccordion({
  plans,
}: GeneralInsurancePlansAccordionProps) {
  const groupId = useId();
            ...
            >
              ...
              <div className="mt-auto">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-2 lg:flex-nowrap lg:gap-3">
                  ...
                  <Link
                    href={scheduleConsultation.href}
                    className="inline-flex shrink-0 items-center justify-center whitespace-nowrap rounded-full border border-white/30 bg-white/10 px-3 py-1.5 text-xs font-semibold text-white backdrop-blur-sm transition-all duration-300 hover:border-white/50 hover:bg-white/15 hover:scale-[1.02] lg:px-3.5 lg:py-2"
                    onClick={(event) => event.stopPropagation()}
                  >
                    Schedule Consultant
                  </Link>
